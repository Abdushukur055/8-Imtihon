...
import { DeleteUser } from "@/app/api/api-service/user.service";
...
const CardItem = ({ item, setModalOpen,modalOpen, setOpen }: any) => {
  ...
  return (
    <Card
      sx={{ maxWidth: 345, minWidth: 350, height: 470 }}
      className="relative"
    >
      <CardHeader
        avatar={
        ...
        action={
          <IconButton aria-label="settings" className="flex flex-col">
            <MoreVertIcon onClick={() => setActive(!active)} />
          </IconButton>
        }
      ...
      />
      <div
        className={active === false ? `hidden ` : "absolute top-[60px] right-1"}
      >
        <div className=" flex flex-col gap-[5px]">
          <button
            className="p-[7px] bg-red-600 border-none rounded-sm text-white text-[15px]"
            onClick={() => deleteUser(item._id)}
          >
            Delete
          </button>
          <button
            className="p-[7px] bg-blue-600 border-none rounded-sm text-white text-[15px]"
